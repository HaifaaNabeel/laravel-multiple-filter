    ...
    <body class="antialiased">
        <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
            <div class="Products-wrap">
                    ...
                    <form method="GET" action="{{ route('AllProducts') }}">
                        ...
                        <p><strong>Filter Products by Brands:</strong></p>
                            ...
                            @foreach ($Brands as $Brand )
                                <label><input type="checkbox" name="brand[]" value="{{$Brand->id}}" id="{{$Brand->id}}" /> {{$Brand->name}}</label><br>
                            @endforeach
                        <p><strong>Filter Products by Categories:</strong></p>

                            @foreach ($Categories as $Category )
                                <label><input type="checkbox" name="category[]" value="{{$Category->id}}" id="{{$Category->id}}" /> {{$Category->name}}</label><br>
                            @endforeach
                    </form>
                </div>
                ...
                <form>
                    @foreach ($Brands as $Brand )
                        <label><input type="checkbox" name="{{$Brand->name}}" value="{{$Brand->id}}" id="{{$Brand->id}}" /> {{$Brand->name}}</label><br>
                    @endforeach
                </form>
                <p><strong>Filter Products by Categories:</strong></p>
                <form>
                    @foreach ($Categories as $Category )
                        <label><input type="checkbox" name="{{$Category->name}}" value="{{$Category->id}}" id="{{$Category->id}}" /> {{$Category->name}}</label><br>
                    @endforeach
                </form> --}}
              ...
              <div class="Products">
                    @foreach ($Products as $Product )
                         <div class="Product" data-id="aloe" data-category="{{$Product->category->name}} {{$Product->brand->name}}">
                            <h2>{{$Product->name}}</h2>
