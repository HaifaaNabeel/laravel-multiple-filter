    ...
    <body class="antialiased">
        <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
            <div class="Products-wrap">
                ...
                <p><strong>Filter Products by Brands:</strong></p>
                <form>
                    {{-- <label><input type="checkbox" name="Brand" value="all" id="all-Brand" class="select-all"/> All</label><br> --}}
                    @foreach ($Brands as $Brand )
                        <label><input type="checkbox" name="Brand" value="{{$Brand->name}}" id="{{$Brand->name}}" /> {{$Brand->name}}</label><br>
                    @endforeach
                </form>
                <p><strong>Filter Products by Categories:</strong></p>
                <form>
                    {{-- <label><input type="checkbox" name="Category" value="all" id="all-Category" class="select-all" /> All</label><br> --}}
                    @foreach ($Categories as $Category )
                        <label><input type="checkbox" name="Category" value="{{$Category->name}}" id="{{$Category->name}}" /> {{$Category->name}}</label><br>
                    @endforeach
                </form>
              ...
              <div class="Products">
                    @foreach ($Products as $Product )
                         <div class="Product" data-id="{{$Product->name}}" data-category="{{$Product->category->name}} {{$Product->brand->name}}">
                            <h2>{{$Product->name}}</h2>
        ...
        <script>
            ...
            // إضافة معالج للنقر على الزر "All"
            ...
            //     var searchValue = $quickSearch.val().toLowerCase().trim(); // قيمة البحث المدخلة

            //     $products.hide().filter(function() {
            //     var productName = $(this).find('h2').text().toLowerCase();

            //     // التحقق من تطابق قيمة البحث مع اسم المنتج
            ...
            // $quickSearch.on('keyup', searchFunc); // استجابة لحدث الكتابة في العنصر السريع
            ...
            var searchFunc = function() {
                ...
                // التحقق من تطابق قيمة البحث مع محتوى العنصر الكامل
